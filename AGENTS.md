...
- Configuration lives in `crates/fxcursor-protocol` (`AppConfig`, presets, self-healing). `src/lib/bindings.ts` is **generated** by `tauri-specta` on every debug run; `src/lib/presets.ts` keeps the TypeScript defaults mirror for browser preview mode. When you change the Rust struct: give new fields a `#[serde(default)]`, update the defaults in `presets.ts`, run `bun run fixtures`, and let the parity tests (`test/config-parity.test.ts`, `crates/fxcursor-protocol/tests/fixture_parity.rs`) confirm both sides agree.
- The trail physics is `TrailChain` in `crates/fxcursor-render/src/renderer.rs` and is GPU-free on purpose: reproduce motion bugs as unit tests there (stop, reversal, turn scenarios exist) before touching the code, and mirror any change in `src/components/Preview/LivePreview.tsx`.
